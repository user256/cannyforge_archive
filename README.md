...
The first two bullets above (search-box and filter narrowing) are client-side
`archive-filters.js` behaviour that the PHP integration suite can't exercise
directly — they're flagged for browser-automation coverage in ticket 704
(Playwright); until that lands they remain manual checks.

### Accessibility smoke checklist (ticket 609)

Automated axe-core-in-CI coverage for the archive page needs a
browser-automation layer against the real-WordPress instance ticket 603 now
boots — that layer itself is ticket 704 (Playwright), not yet built. Until it
lands, verify these by hand after `composer seed:historic`:
