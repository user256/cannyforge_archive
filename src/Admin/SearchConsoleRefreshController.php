<?php
/**
 * Admin-post handler for manual Search Console cache refresh.
 *
...
/**
 * Owns the manual Search Console cache-refresh action.
 */
final class SearchConsoleRefreshController {
	/**
	 * Admin-post action: refresh Search Console cache.
	 */
	public const ACTION_REFRESH = 'cannyforge_archive_google_refresh';

	/**
	 * Nonce field for the refresh action.
	 */
	public const REFRESH_NONCE_FIELD = 'cannyforge_archive_google_refresh_nonce';

	/**
	 * Nonce action for the refresh action.
	 */
	public const REFRESH_NONCE_ACTION = 'cannyforge_archive_google_refresh';
	...
	/**
	 * Search Console refresher.
	 *
	 * @var SearchConsoleTopContentRefresher
	 */
	private SearchConsoleTopContentRefresher $refresher;
	 ...
	 * @param SearchConsoleTopContentRefresher $refresher           Search Console refresher.
	 */
	public function __construct(
		SettingsRepositoryInterface $settings_repository,
		GoogleSettingsStore $google_settings,
		GoogleTokenStore $tokens,
		SearchConsoleTopContentRefresher $refresher
	) {
	...
	/**
	 * Refresh the cached Search Console top post IDs.
	 *
	 * @return void
	 */
	public function refresh(): void {
		...
		if ( ! $this->can_refresh() ) {
			$this->redirect_to_settings(
				__( 'Connect Google and save a Search Console site URL before refreshing.', 'cannyforge-archive' ),
		...
		$this->redirect_to_settings(
			sprintf(
				...
				__( 'Search Console cache refreshed: %d post IDs.', 'cannyforge-archive' ),
				count( $ids )
			),
	...
	/**
	 * Whether the plugin is currently configured to refresh Search Console data.
	 *
	 * @return bool
	 */
	private function can_refresh(): bool {
		return '' !== $this->google_settings->get()->search_console_site_url()
